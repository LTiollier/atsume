...
To guarantee consistency between the host and containers (especially for PostgreSQL database access), all PHP commands (artisan, vendor/bin/pest, pint, stan) **must** be executed via Docker:

*   **Golden Rule:** Always prefix with `docker-compose exec backend`.
*   **Examples:**
    *   `docker-compose exec backend php artisan migrate`
    *   `docker-compose exec backend ./vendor/bin/pest`
    *   `docker-compose exec backend ./vendor/bin/phpstan analyse`
*   **Exception:** If Docker is unavailable or for purely local tools not requiring the DB, but consistently prioritize Docker.
