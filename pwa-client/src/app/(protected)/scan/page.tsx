    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-3xl mx-auto">
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-6 rounded-3xl relative overflow-hidden">
        <div className="absolute top-0 right-0 p-8 opacity-10 scale-150 rotate-12 pointer-events-none">
          <ScanBarcode className="h-40 w-40 text-primary" />
                </div>

        <div className="flex items-center gap-4 relative z-10">
          <div className="h-14 w-14 bg-gradient-to-br from-primary to-indigo-600 rounded-2xl flex items-center justify-center shadow-lg shadow-primary/20">
        ...
        <div className="relative z-10 w-full md:w-auto">
                    {!isScanning ? (
                        <Button
              className={`${isOffline ? 'bg-slate-800 text-slate-500' : 'bg-primary hover:bg-primary text-white'} w-full md:w-auto h-12 px-8 font-bold text-lg rounded-xl shadow-xl transition-all active:scale-95`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="animate-in fade-in zoom-in-95 duration-300">
            ...
            <p className="text-center text-slate-500 text-sm mt-4 font-medium">Placez le code-barres ISBN (EAN-13) au centre de l&apos;image.</p>
                    </div>
                )}
