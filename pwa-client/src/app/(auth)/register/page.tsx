...
export default function RegisterPage() {
  ...
  return (
  <div className="min-h-screen flex items-center justify-center bg-[conic-gradient(at_top_left,_var(--tw-gradient-stops))] from-slate-900 via-primary to-slate-900 p-4 sm:p-6 md:p-8">
    ...
    <Card className="border-slate-800 bg-slate-950/80 backdrop-blur-xl shadow-2xl overflow-hidden">
     ...
     <CardHeader className="space-y-1 pb-6 text-center">
      <div className="flex justify-center mb-4">
       <div className="p-3 bg-primary/10 rounded-2xl ring-1 ring-primary/20">
        <LucideUserPlus className="h-8 w-8 text-primary" />
              </div>
            </div>
      ...
      <CardDescription className="text-slate-400">
       ...
       <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
           ...
           <FormLabel className="text-slate-200">Nom / Pseudo</FormLabel>
            ...
            <div className="relative">
             <LucideUser className="absolute left-3 top-2.5 h-4 w-4 text-slate-500" />
              ...
              className="pl-10 bg-slate-900/50 border-slate-700 text-white placeholder:text-slate-600 focus-visible:ring-primary hover:border-slate-600 transition-colors"
                            {...field}
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-4">
         ...
         className="w-full bg-primary hover:bg-primary text-white font-semibold h-11 transition-all duration-200 active:scale-[0.98] mt-6"
                  disabled={isLoading}
                >
                  {isLoading ? (
                    <>
     ...
     <CardFooter className="flex flex-col space-y-4 border-t border-slate-800/50 bg-slate-900/30 py-6">
      <div className="text-sm text-center text-slate-400">
       ...
       <Link href="/login" className="text-primary hover:text-primary font-medium hover:underline transition-all">
                Se connecter
              </Link>
            </div>
          </CardFooter>
