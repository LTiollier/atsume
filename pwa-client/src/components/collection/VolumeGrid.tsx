...
export function VolumeGrid({
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            ...
            className={`
                            ...
                            ${vol.isPossessed ? (isLoanSelected ? 'ring-4 ring-blue-500 scale-95' : 'ring-2 ring-primary border-none') : (!isReadOnly ? 'hover:scale-105' : '')}
            ...
            <div className="absolute top-0 inset-x-0 p-2 bg-gradient-to-b from-black/80 to-transparent flex justify-between items-start">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                  ...
                  <div className="bg-black/40 text-white rounded-full p-1 opacity-0 hover:opacity-100 transition-opacity">
                    <Circle className="h-8 w-8" />
                        ...
                        {vol.isPossessed && !vol.manga?.is_loaned && !isReadOnly && (
                            <div
                className={`absolute bottom-2 left-2 ${isLoanSelected ? 'bg-blue-600 text-white' : 'bg-slate-900/90 text-primary'} rounded-full p-2 shadow-lg hover:bg-blue-600 hover:text-white transition-colors cursor-pointer border ${isLoanSelected ? 'border-blue-500' : 'border-primary/20'}`}
                                onClick={(e) => {
                                    e.stopPropagation();
                                    onLoanClick?.(vol);
                                }}
                ...
                <ArrowLeftRight className="h-4 w-4" />
                        ...
                        {vol.isPossessed && (
              <div className={`absolute bottom-2 right-2 ${vol.manga?.is_loaned ? 'bg-orange-500' : 'bg-primary'} text-white rounded-full p-1 shadow-lg`}>
